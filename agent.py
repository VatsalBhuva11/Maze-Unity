...
import torch
import numpy as np
from dqntorch import DQN  # Import your custom DQN model
...
class Agent:
    def __init__(self, env):
        ...
        self.model = DQN(self.state_size, self.action_size, self.device)
        ...
        if self.env.is_valid_position(new_position):
            ...
            if np.array_equal(self.position, self.env.destination):
                ...
                if self.game_steps >= 75:
                    ...
                    # Additional logic for incremental rewards can be added here
                    reward += 0.10  # Example reward for progressing
        else:
            reward = -0.75
            terminated = True
            self.game_steps = 0
    ...
    def train(self, episodes, render):
        ...
        for episode in range(episodes):
            state = self.reset()
            done, returns, step, loss = False, 0, 0, 0.0
            while True:
